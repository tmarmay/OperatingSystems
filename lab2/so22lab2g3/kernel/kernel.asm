...
000000008000008e <start>:
{
  ...
  x &= ~MSTATUS_MPP_MASK;
    ...
    8000009c:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffdc9a7>
...
kfree(void *pa)
{
  ...
  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    ...
    80000a16:	44678793          	addi	a5,a5,1094 # 80021e58 <end>
...
0000000080000abe <kinit>:
{
  ...
  freerange(end, (void*)PHYSTOP);
    ...
    80000ae6:	37650513          	addi	a0,a0,886 # 80021e58 <end>
...
main()
{
  ...
  scheduler();        
    ...
    80000f8e:	ff8080e7          	jalr	-8(ra) # 80005f82 <virtio_disk_init>
    userinit();      // first user process
...
proc_mapstacks(pagetable_t kpgtbl)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001884:	130a0a13          	addi	s4,s4,304 # 800169b0 <tickslock>
    char *pa = kalloc();
...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001950:	06498993          	addi	s3,s3,100 # 800169b0 <tickslock>
      initlock(&p->lock, "proc");
...
forkret(void)
{
  ...
  if (first) {
    ...
    80001a1a:	e5a7a783          	lw	a5,-422(a5) # 80008870 <first.1678>
  ...
  usertrapret();
    ...
    80001a34:	e407a023          	sw	zero,-448(a5) # 80008870 <first.1678>
    fsinit(ROOTDEV);
...
0000000080001bd0 <allocproc>:
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001be8:	dcc90913          	addi	s2,s2,-564 # 800169b0 <tickslock>
    acquire(&p->lock);
...
0000000080001c9e <userinit>:
{
  ...
  initproc = p;
    ...
    80001cb6:	c4a7bb23          	sd	a0,-938(a5) # 80008908 <initproc>
  uvmfirst(p->pagetable, initcode, sizeof(initcode));
    80001cba:	03400613          	li	a2,52
    80001cbe:	00007597          	auipc	a1,0x7
    80001cc2:	bc258593          	addi	a1,a1,-1086 # 80008880 <initcode>
...
0000000080001d7c <fork>:
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  acquire(&wait_lock);
    ...
    80001e6c:	d3048493          	addi	s1,s1,-720 # 80010b98 <wait_lock>
...
0000000080001eb8 <scheduler>:
{
  ...
  c->proc = 0;
    ...
    80001ed8:	cac70713          	addi	a4,a4,-852 # 80010b80 <pid_lock>
    80001edc:	9756                	add	a4,a4,s5
    80001ede:	02073823          	sd	zero,48(a4)
        swtch(&c->context, &p->context);
    80001ee2:	0000f717          	auipc	a4,0xf
    80001ee6:	cd670713          	addi	a4,a4,-810 # 80010bb8 <cpus+0x8>
    ...
    80001ef6:	c8ea0a13          	addi	s4,s4,-882 # 80010b80 <pid_lock>
    80001efa:	9a3e                	add	s4,s4,a5
    for(p = proc; p < &proc[NPROC]; p++) {
    80001efc:	00015917          	auipc	s2,0x15
    80001f00:	ab490913          	addi	s2,s2,-1356 # 800169b0 <tickslock>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
  ...
  asm volatile("csrw sstatus, %0" : : "r" (x));
    ...
    80001f14:	0a048493          	addi	s1,s1,160 # 80010fb0 <proc>
...
0000000080001f58 <sched>:
{
  ...
  if(mycpu()->noff != 1)
    ...
    80001f84:	c0070713          	addi	a4,a4,-1024 # 80010b80 <pid_lock>
  ...
  intena = mycpu()->intena;
    ...
    80001faa:	bda90913          	addi	s2,s2,-1062 # 80010b80 <pid_lock>
  ...
  swtch(&p->context, &mycpu()->context);
    ...
    80001fc2:	bfa58593          	addi	a1,a1,-1030 # 80010bb8 <cpus+0x8>
...
wakeup(void *chan)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    800020e6:	ece48493          	addi	s1,s1,-306 # 80010fb0 <proc>
    if(p != myproc()){
      acquire(&p->lock);
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    800020f2:	8c290913          	addi	s2,s2,-1854 # 800169b0 <tickslock>
...
0000000080002144 <reparent>:
{
  ...
  for(pp = proc; pp < &proc[NPROC]; pp++){
    ...
    8000215a:	e5a48493          	addi	s1,s1,-422 # 80010fb0 <proc>
      pp->parent = initproc;
    8000215e:	00006a17          	auipc	s4,0x6
    80002162:	7aaa0a13          	addi	s4,s4,1962 # 80008908 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80002166:	00015997          	auipc	s3,0x15
    8000216a:	84a98993          	addi	s3,s3,-1974 # 800169b0 <tickslock>
...
000000008000219e <exit>:
{
  ...
  if(p == initproc)
    ...
    800021be:	74e7b783          	ld	a5,1870(a5) # 80008908 <initproc>
  ...
  acquire(&wait_lock);
    ...
    8000221a:	98248493          	addi	s1,s1,-1662 # 80010b98 <wait_lock>
...
kill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    80002288:	d2c48493          	addi	s1,s1,-724 # 80010fb0 <proc>
    8000228c:	00014997          	auipc	s3,0x14
    80002290:	72498993          	addi	s3,s3,1828 # 800169b0 <tickslock>
    acquire(&p->lock);
...
0000000080002344 <wait>:
{
  ...
  acquire(&wait_lock);
    ...
    8000236c:	83050513          	addi	a0,a0,-2000 # 80010b98 <wait_lock>
    ...
    80002380:	63498993          	addi	s3,s3,1588 # 800169b0 <tickslock>
        havekids = 1;
    80002384:	4a85                	li	s5,1
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002386:	0000fc17          	auipc	s8,0xf
    8000238a:	812c0c13          	addi	s8,s8,-2030 # 80010b98 <wait_lock>
    havekids = 0;
    8000238e:	875e                	mv	a4,s7
    for(pp = proc; pp < &proc[NPROC]; pp++){
    80002390:	0000f497          	auipc	s1,0xf
    80002394:	c2048493          	addi	s1,s1,-992 # 80010fb0 <proc>
    ...
    800023d2:	7ca50513          	addi	a0,a0,1994 # 80010b98 <wait_lock>
    ...
    8000243c:	76050513          	addi	a0,a0,1888 # 80010b98 <wait_lock>
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    80002548:	bc448493          	addi	s1,s1,-1084 # 80011108 <proc+0x158>
    8000254c:	00014917          	auipc	s2,0x14
    80002550:	5bc90913          	addi	s2,s2,1468 # 80016b08 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
...
trapinit(void)
{
  ...
  initlock(&tickslock, "time");
    ...
    8000264a:	36a50513          	addi	a0,a0,874 # 800169b0 <tickslock>
...
clockintr()
{
  ...
  acquire(&tickslock);
    ...
    8000271a:	29a48493          	addi	s1,s1,666 # 800169b0 <tickslock>
  ...
  ticks++;
    ...
    8000272c:	1e850513          	addi	a0,a0,488 # 80008910 <ticks>
...
devintr()
{
  ...
  if((scause & 0x8000000000000000L) &&
    ...
    800027cc:	c38080e7          	jalr	-968(ra) # 80006400 <virtio_disk_intr>
...
sys_sleep(void)
{
  ...
  while(ticks - ticks0 < n){
    ...
    80002cc2:	cf298993          	addi	s3,s3,-782 # 800169b0 <tickslock>
    80002cc6:	00006497          	auipc	s1,0x6
    80002cca:	c4a48493          	addi	s1,s1,-950 # 80008910 <ticks>
    if(killed(myproc())){
  ...
  release(&tickslock);
    ...
    80002cfe:	cb650513          	addi	a0,a0,-842 # 800169b0 <tickslock>
  ...
  return 0;
    ...
    80002d1e:	c9650513          	addi	a0,a0,-874 # 800169b0 <tickslock>
...
sys_uptime(void)
{
  ...
  acquire(&tickslock);
    ...
    80002d66:	c4e50513          	addi	a0,a0,-946 # 800169b0 <tickslock>
    80002d6a:	ffffe097          	auipc	ra,0xffffe
    80002d6e:	e80080e7          	jalr	-384(ra) # 80000bea <acquire>
  xticks = ticks;
    80002d72:	00006497          	auipc	s1,0x6
    80002d76:	b9e4a483          	lw	s1,-1122(s1) # 80008910 <ticks>
  release(&tickslock);
    80002d7a:	00014517          	auipc	a0,0x14
    80002d7e:	c3650513          	addi	a0,a0,-970 # 800169b0 <tickslock>
...
binit(void)
{
  ...
  initlock(&bcache.lock, "bcache");
    ...
    80002dae:	77658593          	addi	a1,a1,1910 # 80008520 <syscalls+0xd0>
    80002db2:	00014517          	auipc	a0,0x14
    80002db6:	c1650513          	addi	a0,a0,-1002 # 800169c8 <bcache>
  ...
  bcache.head.prev = &bcache.head;
    ...
    80002dc6:	c0678793          	addi	a5,a5,-1018 # 8001e9c8 <bcache+0x8000>
    80002dca:	0001c717          	auipc	a4,0x1c
    80002dce:	e6670713          	addi	a4,a4,-410 # 8001ec30 <bcache+0x8268>
    80002dd2:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002dd6:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002dda:	00014497          	auipc	s1,0x14
    80002dde:	c0648493          	addi	s1,s1,-1018 # 800169e0 <bcache+0x18>
    ...
    80002dea:	742a0a13          	addi	s4,s4,1858 # 80008528 <syscalls+0xd8>
    b->next = bcache.head.next;
...
bread(uint dev, uint blockno)
{
  ...
  acquire(&bcache.lock);
    ...
    80002e3e:	b8e50513          	addi	a0,a0,-1138 # 800169c8 <bcache>
    80002e42:	ffffe097          	auipc	ra,0xffffe
    80002e46:	da8080e7          	jalr	-600(ra) # 80000bea <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002e4a:	0001c497          	auipc	s1,0x1c
    80002e4e:	e364b483          	ld	s1,-458(s1) # 8001ec80 <bcache+0x82b8>
    80002e52:	0001c797          	auipc	a5,0x1c
    80002e56:	dde78793          	addi	a5,a5,-546 # 8001ec30 <bcache+0x8268>
    ...
    80002e7e:	b4e50513          	addi	a0,a0,-1202 # 800169c8 <bcache>
  ...
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    ...
    80002e9c:	de04b483          	ld	s1,-544(s1) # 8001ec78 <bcache+0x82b0>
    80002ea0:	0001c797          	auipc	a5,0x1c
    80002ea4:	d9078793          	addi	a5,a5,-624 # 8001ec30 <bcache+0x8268>
  ...
  panic("bget: no buffers");
    ...
    80002ebc:	67850513          	addi	a0,a0,1656 # 80008530 <syscalls+0xe0>
    ...
    80002edc:	af050513          	addi	a0,a0,-1296 # 800169c8 <bcache>
  ...
  return b;
    ...
    80002f10:	266080e7          	jalr	614(ra) # 80006172 <virtio_disk_rw>
    b->valid = 1;
...
bwrite(struct buf *b)
{
  ...
  virtio_disk_rw(b, 1);
    ...
    80002f3a:	23c080e7          	jalr	572(ra) # 80006172 <virtio_disk_rw>
}
    ...
    80002f4c:	60050513          	addi	a0,a0,1536 # 80008548 <syscalls+0xf8>
...
brelse(struct buf *b)
{
  ...
  acquire(&bcache.lock);
    ...
    80002f84:	a4850513          	addi	a0,a0,-1464 # 800169c8 <bcache>
  ...
  if (b->refcnt == 0) {
    ...
    80002fac:	a2078793          	addi	a5,a5,-1504 # 8001e9c8 <bcache+0x8000>
    80002fb0:	2b87b703          	ld	a4,696(a5)
    80002fb4:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002fb6:	0001c717          	auipc	a4,0x1c
    80002fba:	c7a70713          	addi	a4,a4,-902 # 8001ec30 <bcache+0x8268>
  ...
  release(&bcache.lock);
    ...
    80002fce:	9fe50513          	addi	a0,a0,-1538 # 800169c8 <bcache>
    ...
    80002fea:	56a50513          	addi	a0,a0,1386 # 80008550 <syscalls+0x100>
...
bpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    ...
    80003006:	9c650513          	addi	a0,a0,-1594 # 800169c8 <bcache>
  ...
  release(&bcache.lock);
    ...
    8000301c:	9b050513          	addi	a0,a0,-1616 # 800169c8 <bcache>
...
bunpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    ...
    80003042:	98a50513          	addi	a0,a0,-1654 # 800169c8 <bcache>
  ...
  release(&bcache.lock);
    ...
    80003058:	97450513          	addi	a0,a0,-1676 # 800169c8 <bcache>
...
bfree(int dev, uint b)
{
  ...
  bp = bread(dev, BBLOCK(b, sb));
    ...
    80003084:	0247a783          	lw	a5,36(a5) # 8001f0a4 <sb+0x1c>
  ...
  brelse(bp);
    ...
    800030de:	47e50513          	addi	a0,a0,1150 # 80008558 <syscalls+0x108>
...
00000000800030ea <balloc>:
{
  ...
  for(b = 0; b < sb.size; b += BPB){
    ...
    80003108:	f887a783          	lw	a5,-120(a5) # 8001f08c <sb+0x4>
    ...
    80003118:	f74b0b13          	addi	s6,s6,-140 # 8001f088 <sb>
  ...
  printf("balloc: out of blocks\n");
    ...
    80003212:	36250513          	addi	a0,a0,866 # 80008570 <syscalls+0x120>
...
bmap(struct inode *ip, uint bn)
{
  ...
  panic("bmap: out of range");
    ...
    800032fe:	28e50513          	addi	a0,a0,654 # 80008588 <syscalls+0x138>
...
000000008000330a <iget>:
{
  ...
  acquire(&itable.lock);
    ...
    80003322:	d8a50513          	addi	a0,a0,-630 # 8001f0a8 <itable>
  ...
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    ...
    80003334:	d9048493          	addi	s1,s1,-624 # 8001f0c0 <itable+0x18>
    80003338:	0001e697          	auipc	a3,0x1e
    8000333c:	81868693          	addi	a3,a3,-2024 # 80020b50 <log>
  ...
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    ...
    80003368:	d4450513          	addi	a0,a0,-700 # 8001f0a8 <itable>
  ...
  release(&itable.lock);
    ...
    80003398:	d1450513          	addi	a0,a0,-748 # 8001f0a8 <itable>
    ...
    800033ba:	1ea50513          	addi	a0,a0,490 # 800085a0 <syscalls+0x150>
...
fsinit(int dev) {
  ...
  memmove(sb, bp->data, sizeof(*sb));
    ...
    800033e6:	ca698993          	addi	s3,s3,-858 # 8001f088 <sb>
  ...
  initlog(dev, &sb);
    ...
    8000341a:	c7258593          	addi	a1,a1,-910 # 8001f088 <sb>
    ...
    8000343a:	17a50513          	addi	a0,a0,378 # 800085b0 <syscalls+0x160>
...
0000000080003446 <iinit>:
{
  ...
  initlock(&itable.lock, "itable");
    ...
    80003458:	17458593          	addi	a1,a1,372 # 800085c8 <syscalls+0x178>
    8000345c:	0001c517          	auipc	a0,0x1c
    80003460:	c4c50513          	addi	a0,a0,-948 # 8001f0a8 <itable>
    80003464:	ffffd097          	auipc	ra,0xffffd
    80003468:	6f6080e7          	jalr	1782(ra) # 80000b5a <initlock>
  for(i = 0; i < NINODE; i++) {
    8000346c:	0001c497          	auipc	s1,0x1c
    80003470:	c6448493          	addi	s1,s1,-924 # 8001f0d0 <itable+0x28>
    80003474:	0001d997          	auipc	s3,0x1d
    80003478:	6ec98993          	addi	s3,s3,1772 # 80020b60 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    8000347c:	00005917          	auipc	s2,0x5
    80003480:	15490913          	addi	s2,s2,340 # 800085d0 <syscalls+0x180>
...
00000000800034a6 <ialloc>:
{
  ...
  for(inum = 1; inum < sb.ninodes; inum++){
    ...
    800034c0:	bd872703          	lw	a4,-1064(a4) # 8001f094 <sb+0xc>
    ...
    800034d4:	bb8a0a13          	addi	s4,s4,-1096 # 8001f088 <sb>
  ...
  printf("ialloc: no inodes\n");
    ...
    8000351e:	0be50513          	addi	a0,a0,190 # 800085d8 <syscalls+0x188>
...
0000000080003578 <iupdate>:
{
  ...
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    ...
    80003590:	b145a583          	lw	a1,-1260(a1) # 8001f0a0 <sb+0x18>
...
0000000080003604 <idup>:
{
  ...
  acquire(&itable.lock);
    ...
    80003614:	a9850513          	addi	a0,a0,-1384 # 8001f0a8 <itable>
  ...
  release(&itable.lock);
    ...
    8000362a:	a8250513          	addi	a0,a0,-1406 # 8001f0a8 <itable>
...
0000000080003642 <ilock>:
{
  ...
  if(ip->valid == 0){
    ...
    80003676:	f7e50513          	addi	a0,a0,-130 # 800085f0 <syscalls+0x1a0>
    ...
    8000368c:	a185a583          	lw	a1,-1512(a1) # 8001f0a0 <sb+0x18>
    ...
    800036f8:	f0450513          	addi	a0,a0,-252 # 800085f8 <syscalls+0x1a8>
...
0000000080003704 <iunlock>:
{
  ...
  releasesleep(&ip->lock);
    ...
    80003744:	ec850513          	addi	a0,a0,-312 # 80008608 <syscalls+0x1b8>
...
00000000800037fc <iput>:
{
  ...
  acquire(&itable.lock);
    ...
    8000380e:	89e50513          	addi	a0,a0,-1890 # 8001f0a8 <itable>
  ...
  release(&itable.lock);
    ...
    8000382c:	88050513          	addi	a0,a0,-1920 # 8001f0a8 <itable>
  ...
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    ...
    80003860:	84c50513          	addi	a0,a0,-1972 # 8001f0a8 <itable>
    ...
    80003896:	81650513          	addi	a0,a0,-2026 # 8001f0a8 <itable>
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
  ...
  for(off = 0; off < dp->size; off += sizeof(de)){
    ...
    80003b54:	ac050513          	addi	a0,a0,-1344 # 80008610 <syscalls+0x1c0>
    80003b58:	ffffd097          	auipc	ra,0xffffd
    80003b5c:	9ec080e7          	jalr	-1556(ra) # 80000544 <panic>
      panic("dirlookup read");
    80003b60:	00005517          	auipc	a0,0x5
    80003b64:	ac850513          	addi	a0,a0,-1336 # 80008628 <syscalls+0x1d8>
...
0000000080003d36 <dirlink>:
{
  ...
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    ...
    80003ddc:	86050513          	addi	a0,a0,-1952 # 80008638 <syscalls+0x1e8>
...
write_head(void)
{
  ...
  struct buf *buf = bread(log.dev, log.start);
    ...
    80003e32:	d2290913          	addi	s2,s2,-734 # 80020b50 <log>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    80003e56:	d2e78793          	addi	a5,a5,-722 # 80020b80 <log+0x30>
    ...
    80003e6a:	d1e60613          	addi	a2,a2,-738 # 80020b84 <log+0x34>
...
0000000080003e9c <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    80003ea0:	ce07a783          	lw	a5,-800(a5) # 80020b7c <log+0x2c>
    80003ea4:	0af05d63          	blez	a5,80003f5e <install_trans+0xc2>
{
    ...
    80003ec2:	cc2a8a93          	addi	s5,s5,-830 # 80020b80 <log+0x30>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003ec6:	4a01                	li	s4,0
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003ec8:	0001d997          	auipc	s3,0x1d
    80003ecc:	c8898993          	addi	s3,s3,-888 # 80020b50 <log>
...
0000000080003f60 <initlog>:
{
  ...
  initlock(&log.lock, "log");
    ...
    80003f76:	bde48493          	addi	s1,s1,-1058 # 80020b50 <log>
    80003f7a:	00004597          	auipc	a1,0x4
    80003f7e:	6ce58593          	addi	a1,a1,1742 # 80008648 <syscalls+0x1f8>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    80003fb6:	bce68693          	addi	a3,a3,-1074 # 80020b80 <log+0x30>
...
recover_from_log(void)
{
  ...
  log.lh.n = 0;
    ...
    80003fea:	b807ab23          	sw	zero,-1130(a5) # 80020b7c <log+0x2c>
...
begin_op(void)
{
  ...
  while(1){
    ...
    8000406a:	aea50513          	addi	a0,a0,-1302 # 80020b50 <log>
...
end_op(void)
{
  ...
  acquire(&log.lock);
    ...
    8000409a:	aba48493          	addi	s1,s1,-1350 # 80020b50 <log>
  ...
  if(log.outstanding == 0){
    ...
    800040be:	a9648493          	addi	s1,s1,-1386 # 80020b50 <log>
...
commit()
{
  ...
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    80004144:	a40a8a93          	addi	s5,s5,-1472 # 80020b80 <log+0x30>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80004148:	0001da17          	auipc	s4,0x1d
    8000414c:	a08a0a13          	addi	s4,s4,-1528 # 80020b50 <log>
  ...
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    800041ce:	9a07a923          	sw	zero,-1614(a5) # 80020b7c <log+0x2c>
...
log_write(struct buf *b)
{
  ...
  acquire(&log.lock);
    ...
    800041ee:	96690913          	addi	s2,s2,-1690 # 80020b50 <log>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    8000422c:	95870713          	addi	a4,a4,-1704 # 80020b80 <log+0x30>
  for (i = 0; i < log.lh.n; i++) {
  ...
  log.lh.block[i] = b->blockno;
    ...
    80004248:	90c78793          	addi	a5,a5,-1780 # 80020b50 <log>
  ...
  if (i == log.lh.n) {  // Add new block to log?
    ...
    80004260:	8f470713          	addi	a4,a4,-1804 # 80020b50 <log>
    ...
    80004270:	3f450513          	addi	a0,a0,1012 # 80008660 <syscalls+0x210>
    80004274:	ffffc097          	auipc	ra,0xffffc
    80004278:	2d0080e7          	jalr	720(ra) # 80000544 <panic>
    panic("log_write outside of trans");
    8000427c:	00004517          	auipc	a0,0x4
    80004280:	3fc50513          	addi	a0,a0,1020 # 80008678 <syscalls+0x228>
  ...
  log.lh.block[i] = b->blockno;
    ...
    80004298:	8bc70713          	addi	a4,a4,-1860 # 80020b50 <log>
  ...
  release(&log.lock);
    ...
    800042aa:	8aa50513          	addi	a0,a0,-1878 # 80020b50 <log>
...
initsleeplock(struct sleeplock *lk, char *name)
{
  ...
  initlock(&lk->lk, "sleep lock");
    ...
    800042d6:	3c658593          	addi	a1,a1,966 # 80008698 <syscalls+0x248>
...
fileinit(void)
{
  ...
  initlock(&ftable.lock, "ftable");
    ...
    800043f8:	2b458593          	addi	a1,a1,692 # 800086a8 <syscalls+0x258>
    800043fc:	0001d517          	auipc	a0,0x1d
    80004400:	89c50513          	addi	a0,a0,-1892 # 80020c98 <ftable>
...
filealloc(void)
{
  ...
  acquire(&ftable.lock);
    ...
    80004422:	87a50513          	addi	a0,a0,-1926 # 80020c98 <ftable>
    80004426:	ffffc097          	auipc	ra,0xffffc
    8000442a:	7c4080e7          	jalr	1988(ra) # 80000bea <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    8000442e:	0001d497          	auipc	s1,0x1d
    80004432:	88248493          	addi	s1,s1,-1918 # 80020cb0 <ftable+0x18>
    80004436:	0001e717          	auipc	a4,0x1e
    8000443a:	81a70713          	addi	a4,a4,-2022 # 80021c50 <array_sem>
    if(f->ref == 0){
  ...
  release(&ftable.lock);
    ...
    8000444e:	84e50513          	addi	a0,a0,-1970 # 80020c98 <ftable>
  ...
  return 0;
    ...
    80004466:	83650513          	addi	a0,a0,-1994 # 80020c98 <ftable>
...
filedup(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    8000448e:	80e50513          	addi	a0,a0,-2034 # 80020c98 <ftable>
  ...
  release(&ftable.lock);
    ...
    800044a8:	7f450513          	addi	a0,a0,2036 # 80020c98 <ftable>
  ...
  return f;
    ...
    800044c4:	1f050513          	addi	a0,a0,496 # 800086b0 <syscalls+0x260>
...
fileclose(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    800044e8:	7b450513          	addi	a0,a0,1972 # 80020c98 <ftable>
  ...
  release(&ftable.lock);
    ...
    80004522:	77a50513          	addi	a0,a0,1914 # 80020c98 <ftable>
...
fileread(struct file *f, uint64 addr, int n)
{
  ...
  return r;
    ...
    8000469e:	55e70713          	addi	a4,a4,1374 # 80020bf8 <devsw>
    ...
    800046b4:	01850513          	addi	a0,a0,24 # 800086c8 <syscalls+0x278>
...
filewrite(struct file *f, uint64 addr, int n)
{
  ...
  if(f->type == FD_PIPE){
    ...
    8000473e:	4be70713          	addi	a4,a4,1214 # 80020bf8 <devsw>
  ...
  return ret;
    ...
    800047e8:	ef450513          	addi	a0,a0,-268 # 800086d8 <syscalls+0x288>
...
pipealloc(struct file **f0, struct file **f1)
{
  ...
  *f0 = *f1 = 0;
  ...
  initlock(&pi->lock, "pipe");
    ...
    80004858:	e9458593          	addi	a1,a1,-364 # 800086e8 <syscalls+0x298>
...
exec(char *path, char **argv)
{
 ...
 bad:
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    80004c4c:	aa850513          	addi	a0,a0,-1368 # 800086f0 <syscalls+0x2a0>
...
fdalloc(struct file *f)
{
  ...
  for(fd = 0; fd < NOFILE; fd++){
    80004f6e:	0d050793          	addi	a5,a0,208 # fffffffffffff0d0 <end+0xffffffff7ffdd278>
...
create(char *path, short type, short major, short minor)
{
  ...
  return ip;
    ...
    800050a8:	66c58593          	addi	a1,a1,1644 # 80008710 <syscalls+0x2c0>
    ...
    800050c0:	65c58593          	addi	a1,a1,1628 # 80008718 <syscalls+0x2c8>
...
0000000080005402 <sys_unlink>:
{
  ...
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    ...
    8000544e:	2c658593          	addi	a1,a1,710 # 80008710 <syscalls+0x2c0>
    ...
    80005466:	2b658593          	addi	a1,a1,694 # 80008718 <syscalls+0x2c8>
  ...
  return 0;
    ...
    80005526:	1fe50513          	addi	a0,a0,510 # 80008720 <syscalls+0x2d0>
  ...
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    ...
    80005572:	1ca50513          	addi	a0,a0,458 # 80008738 <syscalls+0x2e8>
    80005576:	ffffb097          	auipc	ra,0xffffb
    8000557a:	fce080e7          	jalr	-50(ra) # 80000544 <panic>
    panic("unlink: writei");
    8000557e:	00003517          	auipc	a0,0x3
    80005582:	1d250513          	addi	a0,a0,466 # 80008750 <syscalls+0x300>
...
uint64 sys_sem_open(void)
{
    ...
    80005cb6:	f9e70713          	addi	a4,a4,-98 # 80021c50 <array_sem>
    ...
    80005cd2:	a9258593          	addi	a1,a1,-1390 # 80008760 <syscalls+0x310>
    ...
    80005ce8:	639c                	ld	a5,0(a5)
    80005cea:	fd842703          	lw	a4,-40(s0)
    80005cee:	c398                	sw	a4,0(a5)
	array_sem[sem]->kill = false;
    80005cf0:	fdc42783          	lw	a5,-36(s0)
    80005cf4:	078e                	slli	a5,a5,0x3
    80005cf6:	94be                	add	s1,s1,a5
    80005cf8:	609c                	ld	a5,0(s1)
    80005cfa:	02078023          	sb	zero,32(a5) # c201020 <_entry-0x73dfefe0>
	return 1;
    80005cfe:	4505                	li	a0,1
    80005d00:	bf71                	j	80005c9c <sys_sem_open+0x36>

0000000080005d02 <sys_sem_close>:
...
uint64 sys_sem_close(void)
{
	...
	argint(0, &sem);
    80005d0c:	fdc40593          	addi	a1,s0,-36
    80005d10:	4501                	li	a0,0
    80005d12:	ffffd097          	auipc	ra,0xffffd
    80005d16:	dc4080e7          	jalr	-572(ra) # 80002ad6 <argint>

	/* Semaforo valido */
	acquire(&array_sem[sem]->sl);
    80005d1a:	0001c497          	auipc	s1,0x1c
    80005d1e:	f3648493          	addi	s1,s1,-202 # 80021c50 <array_sem>
    80005d22:	fdc42783          	lw	a5,-36(s0)
    80005d26:	078e                	slli	a5,a5,0x3
    80005d28:	97a6                	add	a5,a5,s1
    80005d2a:	6388                	ld	a0,0(a5)
    80005d2c:	0521                	addi	a0,a0,8
    80005d2e:	ffffb097          	auipc	ra,0xffffb
    80005d32:	ebc080e7          	jalr	-324(ra) # 80000bea <acquire>
	if (array_sem[sem]->value == 0)
    80005d36:	fdc42783          	lw	a5,-36(s0)
    80005d3a:	00379713          	slli	a4,a5,0x3
    80005d3e:	94ba                	add	s1,s1,a4
    80005d40:	6088                	ld	a0,0(s1)
    80005d42:	4118                	lw	a4,0(a0)
    80005d44:	ef1d                	bnez	a4,80005d82 <sys_sem_close+0x80>
	{
		release(&array_sem[sem]->sl);
    80005d46:	0521                	addi	a0,a0,8
    80005d48:	ffffb097          	auipc	ra,0xffffb
    80005d4c:	f56080e7          	jalr	-170(ra) # 80000c9e <release>
		kfree((struct semaphore_t *)array_sem[sem]);
    80005d50:	0001c497          	auipc	s1,0x1c
    80005d54:	f0048493          	addi	s1,s1,-256 # 80021c50 <array_sem>
    80005d58:	fdc42783          	lw	a5,-36(s0)
    80005d5c:	078e                	slli	a5,a5,0x3
    80005d5e:	97a6                	add	a5,a5,s1
    80005d60:	6388                	ld	a0,0(a5)
    80005d62:	ffffb097          	auipc	ra,0xffffb
    80005d66:	c9c080e7          	jalr	-868(ra) # 800009fe <kfree>
		array_sem[sem] = NULL;
    80005d6a:	fdc42783          	lw	a5,-36(s0)
    80005d6e:	078e                	slli	a5,a5,0x3
    80005d70:	94be                	add	s1,s1,a5
    80005d72:	0004b023          	sd	zero,0(s1)
		return 1;
    ...
    80005d7c:	64e2                	ld	s1,24(sp)
    80005d7e:	6145                	addi	sp,sp,48
    80005d80:	8082                	ret
		array_sem[sem]->kill = true;
    80005d82:	4705                	li	a4,1
    80005d84:	02e50023          	sb	a4,32(a0) # c201020 <_entry-0x73dfefe0>
		release(&array_sem[sem]->sl);
    80005d88:	078e                	slli	a5,a5,0x3
    80005d8a:	0001c717          	auipc	a4,0x1c
    80005d8e:	ec670713          	addi	a4,a4,-314 # 80021c50 <array_sem>
    80005d92:	97ba                	add	a5,a5,a4
    80005d94:	6388                	ld	a0,0(a5)
    80005d96:	0521                	addi	a0,a0,8
    80005d98:	ffffb097          	auipc	ra,0xffffb
    80005d9c:	f06080e7          	jalr	-250(ra) # 80000c9e <release>
		return 0;
    80005da0:	4501                	li	a0,0
    80005da2:	bfd9                	j	80005d78 <sys_sem_close+0x76>

0000000080005da4 <sys_sem_up>:

uint64 sys_sem_up(void)
{
    80005da4:	7179                	addi	sp,sp,-48
    80005da6:	f406                	sd	ra,40(sp)
    80005da8:	f022                	sd	s0,32(sp)
    80005daa:	ec26                	sd	s1,24(sp)
    80005dac:	1800                	addi	s0,sp,48
	int sem;

	argint(0, &sem);
    80005dae:	fdc40593          	addi	a1,s0,-36
    80005db2:	4501                	li	a0,0
    80005db4:	ffffd097          	auipc	ra,0xffffd
    80005db8:	d22080e7          	jalr	-734(ra) # 80002ad6 <argint>
	acquire(&array_sem[sem]->sl);
    80005dbc:	0001c497          	auipc	s1,0x1c
    80005dc0:	e9448493          	addi	s1,s1,-364 # 80021c50 <array_sem>
    80005dc4:	fdc42783          	lw	a5,-36(s0)
    80005dc8:	078e                	slli	a5,a5,0x3
    80005dca:	97a6                	add	a5,a5,s1
    80005dcc:	6388                	ld	a0,0(a5)
    80005dce:	0521                	addi	a0,a0,8
    80005dd0:	ffffb097          	auipc	ra,0xffffb
    80005dd4:	e1a080e7          	jalr	-486(ra) # 80000bea <acquire>

	if (array_sem[sem]->value == 0)
    80005dd8:	fdc42503          	lw	a0,-36(s0)
    80005ddc:	00351793          	slli	a5,a0,0x3
    80005de0:	94be                	add	s1,s1,a5
    80005de2:	609c                	ld	a5,0(s1)
    80005de4:	439c                	lw	a5,0(a5)
    80005de6:	cf8d                	beqz	a5,80005e20 <sys_sem_up+0x7c>
	{
		wakeup(&array_sem[sem]);
	}
	array_sem[sem]->value++;
    80005de8:	0001c717          	auipc	a4,0x1c
    80005dec:	e6870713          	addi	a4,a4,-408 # 80021c50 <array_sem>
    80005df0:	fdc42783          	lw	a5,-36(s0)
    80005df4:	078e                	slli	a5,a5,0x3
    80005df6:	97ba                	add	a5,a5,a4
    80005df8:	6394                	ld	a3,0(a5)
    80005dfa:	429c                	lw	a5,0(a3)
    80005dfc:	2785                	addiw	a5,a5,1
    80005dfe:	c29c                	sw	a5,0(a3)

	release(&array_sem[sem]->sl);
    80005e00:	fdc42783          	lw	a5,-36(s0)
    80005e04:	078e                	slli	a5,a5,0x3
    80005e06:	973e                	add	a4,a4,a5
    80005e08:	6308                	ld	a0,0(a4)
    80005e0a:	0521                	addi	a0,a0,8
    80005e0c:	ffffb097          	auipc	ra,0xffffb
    80005e10:	e92080e7          	jalr	-366(ra) # 80000c9e <release>
	return 1;
}
    80005e14:	4505                	li	a0,1
    80005e16:	70a2                	ld	ra,40(sp)
    80005e18:	7402                	ld	s0,32(sp)
    80005e1a:	64e2                	ld	s1,24(sp)
    80005e1c:	6145                	addi	sp,sp,48
    80005e1e:	8082                	ret
		wakeup(&array_sem[sem]);
    80005e20:	8526                	mv	a0,s1
    80005e22:	ffffc097          	auipc	ra,0xffffc
    80005e26:	2ac080e7          	jalr	684(ra) # 800020ce <wakeup>
    80005e2a:	bf7d                	j	80005de8 <sys_sem_up+0x44>

0000000080005e2c <sys_sem_down>:

uint64 sys_sem_down(void)
{
    80005e2c:	7179                	addi	sp,sp,-48
    80005e2e:	f406                	sd	ra,40(sp)
    80005e30:	f022                	sd	s0,32(sp)
    80005e32:	ec26                	sd	s1,24(sp)
    ...
    80005e40:	c9a080e7          	jalr	-870(ra) # 80002ad6 <argint>
	acquire(&array_sem[sem]->sl);
    80005e44:	0001c497          	auipc	s1,0x1c
    80005e48:	e0c48493          	addi	s1,s1,-500 # 80021c50 <array_sem>
    80005e4c:	fdc42783          	lw	a5,-36(s0)
    80005e50:	078e                	slli	a5,a5,0x3
    80005e52:	97a6                	add	a5,a5,s1
    80005e54:	6388                	ld	a0,0(a5)
    80005e56:	0521                	addi	a0,a0,8
    80005e58:	ffffb097          	auipc	ra,0xffffb
    80005e5c:	d92080e7          	jalr	-622(ra) # 80000bea <acquire>

	if (array_sem[sem]->value == 0)
    80005e60:	fdc42783          	lw	a5,-36(s0)
    80005e64:	00379713          	slli	a4,a5,0x3
    80005e68:	94ba                	add	s1,s1,a4
    80005e6a:	6088                	ld	a0,0(s1)
    80005e6c:	4118                	lw	a4,0(a0)
    80005e6e:	e305                	bnez	a4,80005e8e <sys_sem_down+0x62>
	{
		if (array_sem[sem]->kill)
    80005e70:	02054703          	lbu	a4,32(a0)
    80005e74:	e32d                	bnez	a4,80005ed6 <sys_sem_down+0xaa>
			kfree((struct semaphore_t *)array_sem[sem]);
			array_sem[sem] = NULL;
			release(&array_sem[sem]->sl);
			return 1;
    ...
    80005e78:	00850593          	addi	a1,a0,8
    80005e7c:	0001c517          	auipc	a0,0x1c
    80005e80:	dd450513          	addi	a0,a0,-556 # 80021c50 <array_sem>
    80005e84:	953e                	add	a0,a0,a5
    80005e86:	ffffc097          	auipc	ra,0xffffc
    80005e8a:	1e4080e7          	jalr	484(ra) # 8000206a <sleep>
	}
	
	if (array_sem[sem]->value >= 1)
    80005e8e:	fdc42783          	lw	a5,-36(s0)
    80005e92:	00379713          	slli	a4,a5,0x3
    80005e96:	0001c797          	auipc	a5,0x1c
    80005e9a:	dba78793          	addi	a5,a5,-582 # 80021c50 <array_sem>
    80005e9e:	97ba                	add	a5,a5,a4
    80005ea0:	6398                	ld	a4,0(a5)
    80005ea2:	431c                	lw	a5,0(a4)
    80005ea4:	00f05463          	blez	a5,80005eac <sys_sem_down+0x80>
	{
		array_sem[sem]->value--;
    80005ea8:	37fd                	addiw	a5,a5,-1
    80005eaa:	c31c                	sw	a5,0(a4)
	}
	

	release(&array_sem[sem]->sl);
    80005eac:	fdc42783          	lw	a5,-36(s0)
    80005eb0:	00379713          	slli	a4,a5,0x3
    80005eb4:	0001c797          	auipc	a5,0x1c
    80005eb8:	d9c78793          	addi	a5,a5,-612 # 80021c50 <array_sem>
    80005ebc:	97ba                	add	a5,a5,a4
    80005ebe:	6388                	ld	a0,0(a5)
    80005ec0:	0521                	addi	a0,a0,8
    80005ec2:	ffffb097          	auipc	ra,0xffffb
    80005ec6:	ddc080e7          	jalr	-548(ra) # 80000c9e <release>
	return 1;
    ...
    80005eda:	b28080e7          	jalr	-1240(ra) # 800009fe <kfree>
			array_sem[sem] = NULL;
    80005ede:	fdc42783          	lw	a5,-36(s0)
    80005ee2:	00379713          	slli	a4,a5,0x3
    80005ee6:	0001c797          	auipc	a5,0x1c
    80005eea:	d6a78793          	addi	a5,a5,-662 # 80021c50 <array_sem>
    80005eee:	97ba                	add	a5,a5,a4
    80005ef0:	0007b023          	sd	zero,0(a5)
			release(&array_sem[sem]->sl);
    80005ef4:	4521                	li	a0,8
    80005ef6:	ffffb097          	auipc	ra,0xffffb
    80005efa:	da8080e7          	jalr	-600(ra) # 80000c9e <release>
			return 1;
    80005efe:	b7f1                	j	80005eca <sys_sem_down+0x9e>

0000000080005f00 <free_desc>:
}
...
free_desc(int i)
{
  ...
  if(i >= NUM)
    80005f08:	479d                	li	a5,7
    80005f0a:	04a7cc63          	blt	a5,a0,80005f62 <free_desc+0x62>
    panic("free_desc 1");
  if(disk.free[i])
    80005f0e:	0001c797          	auipc	a5,0x1c
    80005f12:	e0a78793          	addi	a5,a5,-502 # 80021d18 <disk>
    80005f16:	97aa                	add	a5,a5,a0
    80005f18:	0187c783          	lbu	a5,24(a5)
    80005f1c:	ebb9                	bnez	a5,80005f72 <free_desc+0x72>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    80005f1e:	00451613          	slli	a2,a0,0x4
    80005f22:	0001c797          	auipc	a5,0x1c
    80005f26:	df678793          	addi	a5,a5,-522 # 80021d18 <disk>
    80005f2a:	6394                	ld	a3,0(a5)
    80005f2c:	96b2                	add	a3,a3,a2
    80005f2e:	0006b023          	sd	zero,0(a3)
  disk.desc[i].len = 0;
    80005f32:	6398                	ld	a4,0(a5)
    80005f34:	9732                	add	a4,a4,a2
    80005f36:	00072423          	sw	zero,8(a4)
  disk.desc[i].flags = 0;
    80005f3a:	00071623          	sh	zero,12(a4)
  disk.desc[i].next = 0;
    80005f3e:	00071723          	sh	zero,14(a4)
  disk.free[i] = 1;
    80005f42:	953e                	add	a0,a0,a5
    80005f44:	4785                	li	a5,1
    80005f46:	00f50c23          	sb	a5,24(a0)
  wakeup(&disk.free[0]);
    80005f4a:	0001c517          	auipc	a0,0x1c
    80005f4e:	de650513          	addi	a0,a0,-538 # 80021d30 <disk+0x18>
    80005f52:	ffffc097          	auipc	ra,0xffffc
    80005f56:	17c080e7          	jalr	380(ra) # 800020ce <wakeup>
}
    ...
    80005f66:	80650513          	addi	a0,a0,-2042 # 80008768 <syscalls+0x318>
    80005f6a:	ffffa097          	auipc	ra,0xffffa
    80005f6e:	5da080e7          	jalr	1498(ra) # 80000544 <panic>
    panic("free_desc 2");
    80005f72:	00003517          	auipc	a0,0x3
    80005f76:	80650513          	addi	a0,a0,-2042 # 80008778 <syscalls+0x328>
...
0000000080005f82 <virtio_disk_init>:
{
  ...
  initlock(&disk.vdisk_lock, "virtio_disk");
    80005f8e:	00002597          	auipc	a1,0x2
    80005f92:	7fa58593          	addi	a1,a1,2042 # 80008788 <syscalls+0x338>
    80005f96:	0001c517          	auipc	a0,0x1c
    80005f9a:	eaa50513          	addi	a0,a0,-342 # 80021e40 <disk+0x128>
    80005f9e:	ffffb097          	auipc	ra,0xffffb
    80005fa2:	bbc080e7          	jalr	-1092(ra) # 80000b5a <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005fa6:	100017b7          	lui	a5,0x10001
    80005faa:	4398                	lw	a4,0(a5)
    80005fac:	2701                	sext.w	a4,a4
    80005fae:	747277b7          	lui	a5,0x74727
    80005fb2:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80005fb6:	14f71e63          	bne	a4,a5,80006112 <virtio_disk_init+0x190>
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    80005fba:	100017b7          	lui	a5,0x10001
    80005fbe:	43dc                	lw	a5,4(a5)
    80005fc0:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005fc2:	4709                	li	a4,2
    80005fc4:	14e79763          	bne	a5,a4,80006112 <virtio_disk_init+0x190>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    80005fc8:	100017b7          	lui	a5,0x10001
    80005fcc:	479c                	lw	a5,8(a5)
    80005fce:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    80005fd0:	14e79163          	bne	a5,a4,80006112 <virtio_disk_init+0x190>
    ...
    80005fe4:	12f71763          	bne	a4,a5,80006112 <virtio_disk_init+0x190>
  *R(VIRTIO_MMIO_STATUS) = status;
    80005fe8:	100017b7          	lui	a5,0x10001
    80005fec:	0607a823          	sw	zero,112(a5) # 10001070 <_entry-0x6fffef90>
  *R(VIRTIO_MMIO_STATUS) = status;
  ...
  *R(VIRTIO_MMIO_STATUS) = status;
  ...
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    80005ff8:	4b94                	lw	a3,16(a5)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    80005ffa:	c7ffe737          	lui	a4,0xc7ffe
    80005ffe:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fdc907>
    80006002:	8f75                	and	a4,a4,a3
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    80006004:	2701                	sext.w	a4,a4
    80006006:	d398                	sw	a4,32(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006008:	472d                	li	a4,11
    8000600a:	dbb8                	sw	a4,112(a5)
  status = *R(VIRTIO_MMIO_STATUS);
    8000600c:	0707a903          	lw	s2,112(a5)
    80006010:	2901                	sext.w	s2,s2
  if(!(status & VIRTIO_CONFIG_S_FEATURES_OK))
    80006012:	00897793          	andi	a5,s2,8
    80006016:	10078663          	beqz	a5,80006122 <virtio_disk_init+0x1a0>
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    8000601a:	100017b7          	lui	a5,0x10001
    8000601e:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  if(*R(VIRTIO_MMIO_QUEUE_READY))
    80006022:	43fc                	lw	a5,68(a5)
    80006024:	2781                	sext.w	a5,a5
    80006026:	10079663          	bnez	a5,80006132 <virtio_disk_init+0x1b0>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    8000602a:	100017b7          	lui	a5,0x10001
    8000602e:	5bdc                	lw	a5,52(a5)
    80006030:	2781                	sext.w	a5,a5
  if(max == 0)
    80006032:	10078863          	beqz	a5,80006142 <virtio_disk_init+0x1c0>
  if(max < NUM)
    80006036:	471d                	li	a4,7
    80006038:	10f77d63          	bgeu	a4,a5,80006152 <virtio_disk_init+0x1d0>
  disk.desc = kalloc();
    8000603c:	ffffb097          	auipc	ra,0xffffb
    80006040:	abe080e7          	jalr	-1346(ra) # 80000afa <kalloc>
    80006044:	0001c497          	auipc	s1,0x1c
    80006048:	cd448493          	addi	s1,s1,-812 # 80021d18 <disk>
    8000604c:	e088                	sd	a0,0(s1)
  disk.avail = kalloc();
    8000604e:	ffffb097          	auipc	ra,0xffffb
    80006052:	aac080e7          	jalr	-1364(ra) # 80000afa <kalloc>
    80006056:	e488                	sd	a0,8(s1)
  disk.used = kalloc();
    80006058:	ffffb097          	auipc	ra,0xffffb
    8000605c:	aa2080e7          	jalr	-1374(ra) # 80000afa <kalloc>
    80006060:	87aa                	mv	a5,a0
    80006062:	e888                	sd	a0,16(s1)
  if(!disk.desc || !disk.avail || !disk.used)
    80006064:	6088                	ld	a0,0(s1)
    80006066:	cd75                	beqz	a0,80006162 <virtio_disk_init+0x1e0>
    80006068:	0001c717          	auipc	a4,0x1c
    8000606c:	cb873703          	ld	a4,-840(a4) # 80021d20 <disk+0x8>
    80006070:	cb6d                	beqz	a4,80006162 <virtio_disk_init+0x1e0>
    80006072:	cbe5                	beqz	a5,80006162 <virtio_disk_init+0x1e0>
  memset(disk.desc, 0, PGSIZE);
    80006074:	6605                	lui	a2,0x1
    80006076:	4581                	li	a1,0
    80006078:	ffffb097          	auipc	ra,0xffffb
    8000607c:	c6e080e7          	jalr	-914(ra) # 80000ce6 <memset>
  memset(disk.avail, 0, PGSIZE);
    80006080:	0001c497          	auipc	s1,0x1c
    80006084:	c9848493          	addi	s1,s1,-872 # 80021d18 <disk>
    80006088:	6605                	lui	a2,0x1
    8000608a:	4581                	li	a1,0
    8000608c:	6488                	ld	a0,8(s1)
    8000608e:	ffffb097          	auipc	ra,0xffffb
    80006092:	c58080e7          	jalr	-936(ra) # 80000ce6 <memset>
  memset(disk.used, 0, PGSIZE);
    80006096:	6605                	lui	a2,0x1
    80006098:	4581                	li	a1,0
    8000609a:	6888                	ld	a0,16(s1)
    8000609c:	ffffb097          	auipc	ra,0xffffb
    800060a0:	c4a080e7          	jalr	-950(ra) # 80000ce6 <memset>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    800060a4:	100017b7          	lui	a5,0x10001
    800060a8:	4721                	li	a4,8
    800060aa:	df98                	sw	a4,56(a5)
  *R(VIRTIO_MMIO_QUEUE_DESC_LOW) = (uint64)disk.desc;
    800060ac:	4098                	lw	a4,0(s1)
    800060ae:	08e7a023          	sw	a4,128(a5) # 10001080 <_entry-0x6fffef80>
  *R(VIRTIO_MMIO_QUEUE_DESC_HIGH) = (uint64)disk.desc >> 32;
    800060b2:	40d8                	lw	a4,4(s1)
    800060b4:	08e7a223          	sw	a4,132(a5)
  *R(VIRTIO_MMIO_DRIVER_DESC_LOW) = (uint64)disk.avail;
    800060b8:	6498                	ld	a4,8(s1)
    800060ba:	0007069b          	sext.w	a3,a4
    800060be:	08d7a823          	sw	a3,144(a5)
  *R(VIRTIO_MMIO_DRIVER_DESC_HIGH) = (uint64)disk.avail >> 32;
    800060c2:	9701                	srai	a4,a4,0x20
    800060c4:	08e7aa23          	sw	a4,148(a5)
  *R(VIRTIO_MMIO_DEVICE_DESC_LOW) = (uint64)disk.used;
    800060c8:	6898                	ld	a4,16(s1)
    800060ca:	0007069b          	sext.w	a3,a4
    800060ce:	0ad7a023          	sw	a3,160(a5)
  *R(VIRTIO_MMIO_DEVICE_DESC_HIGH) = (uint64)disk.used >> 32;
    800060d2:	9701                	srai	a4,a4,0x20
    800060d4:	0ae7a223          	sw	a4,164(a5)
  *R(VIRTIO_MMIO_QUEUE_READY) = 0x1;
    800060d8:	4685                	li	a3,1
    800060da:	c3f4                	sw	a3,68(a5)
    disk.free[i] = 1;
    800060dc:	4705                	li	a4,1
    800060de:	00d48c23          	sb	a3,24(s1)
    800060e2:	00e48ca3          	sb	a4,25(s1)
    800060e6:	00e48d23          	sb	a4,26(s1)
    800060ea:	00e48da3          	sb	a4,27(s1)
    800060ee:	00e48e23          	sb	a4,28(s1)
    800060f2:	00e48ea3          	sb	a4,29(s1)
    800060f6:	00e48f23          	sb	a4,30(s1)
    800060fa:	00e48fa3          	sb	a4,31(s1)
  status |= VIRTIO_CONFIG_S_DRIVER_OK;
    800060fe:	00496913          	ori	s2,s2,4
  *R(VIRTIO_MMIO_STATUS) = status;
    80006102:	0727a823          	sw	s2,112(a5)
}
    80006106:	60e2                	ld	ra,24(sp)
    80006108:	6442                	ld	s0,16(sp)
    8000610a:	64a2                	ld	s1,8(sp)
    8000610c:	6902                	ld	s2,0(sp)
    8000610e:	6105                	addi	sp,sp,32
    80006110:	8082                	ret
    panic("could not find virtio disk");
    80006112:	00002517          	auipc	a0,0x2
    80006116:	68650513          	addi	a0,a0,1670 # 80008798 <syscalls+0x348>
    8000611a:	ffffa097          	auipc	ra,0xffffa
    8000611e:	42a080e7          	jalr	1066(ra) # 80000544 <panic>
    panic("virtio disk FEATURES_OK unset");
    80006122:	00002517          	auipc	a0,0x2
    80006126:	69650513          	addi	a0,a0,1686 # 800087b8 <syscalls+0x368>
    8000612a:	ffffa097          	auipc	ra,0xffffa
    8000612e:	41a080e7          	jalr	1050(ra) # 80000544 <panic>
    panic("virtio disk should not be ready");
    80006132:	00002517          	auipc	a0,0x2
    80006136:	6a650513          	addi	a0,a0,1702 # 800087d8 <syscalls+0x388>
    8000613a:	ffffa097          	auipc	ra,0xffffa
    8000613e:	40a080e7          	jalr	1034(ra) # 80000544 <panic>
    panic("virtio disk has no queue 0");
    80006142:	00002517          	auipc	a0,0x2
    80006146:	6b650513          	addi	a0,a0,1718 # 800087f8 <syscalls+0x3a8>
    8000614a:	ffffa097          	auipc	ra,0xffffa
    8000614e:	3fa080e7          	jalr	1018(ra) # 80000544 <panic>
    panic("virtio disk max queue too short");
    80006152:	00002517          	auipc	a0,0x2
    80006156:	6c650513          	addi	a0,a0,1734 # 80008818 <syscalls+0x3c8>
    8000615a:	ffffa097          	auipc	ra,0xffffa
    8000615e:	3ea080e7          	jalr	1002(ra) # 80000544 <panic>
    panic("virtio disk kalloc");
    80006162:	00002517          	auipc	a0,0x2
    80006166:	6d650513          	addi	a0,a0,1750 # 80008838 <syscalls+0x3e8>
    8000616a:	ffffa097          	auipc	ra,0xffffa
    8000616e:	3da080e7          	jalr	986(ra) # 80000544 <panic>

0000000080006172 <virtio_disk_rw>:
  return 0;
}
...
{
    80006172:	7159                	addi	sp,sp,-112
    80006174:	f486                	sd	ra,104(sp)
    80006176:	f0a2                	sd	s0,96(sp)
    80006178:	eca6                	sd	s1,88(sp)
    8000617a:	e8ca                	sd	s2,80(sp)
    8000617c:	e4ce                	sd	s3,72(sp)
    8000617e:	e0d2                	sd	s4,64(sp)
    80006180:	fc56                	sd	s5,56(sp)
    80006182:	f85a                	sd	s6,48(sp)
    80006184:	f45e                	sd	s7,40(sp)
    80006186:	f062                	sd	s8,32(sp)
    80006188:	ec66                	sd	s9,24(sp)
    8000618a:	e86a                	sd	s10,16(sp)
    8000618c:	1880                	addi	s0,sp,112
    8000618e:	892a                	mv	s2,a0
    80006190:	8d2e                	mv	s10,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    80006192:	00c52c83          	lw	s9,12(a0)
    80006196:	001c9c9b          	slliw	s9,s9,0x1
    8000619a:	1c82                	slli	s9,s9,0x20
    8000619c:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    800061a0:	0001c517          	auipc	a0,0x1c
    800061a4:	ca050513          	addi	a0,a0,-864 # 80021e40 <disk+0x128>
    800061a8:	ffffb097          	auipc	ra,0xffffb
    800061ac:	a42080e7          	jalr	-1470(ra) # 80000bea <acquire>
  for(int i = 0; i < 3; i++){
    800061b0:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    800061b2:	4ba1                	li	s7,8
      disk.free[i] = 0;
    800061b4:	0001cb17          	auipc	s6,0x1c
    800061b8:	b64b0b13          	addi	s6,s6,-1180 # 80021d18 <disk>
  for(int i = 0; i < 3; i++){
    800061bc:	4a8d                	li	s5,3
  for(int i = 0; i < NUM; i++){
    800061be:	8a4e                	mv	s4,s3
  int idx[3];
  while(1){
    ...
    800061c0:	0001cc17          	auipc	s8,0x1c
    800061c4:	c80c0c13          	addi	s8,s8,-896 # 80021e40 <disk+0x128>
    800061c8:	a8b5                	j	80006244 <virtio_disk_rw+0xd2>
      disk.free[i] = 0;
    800061ca:	00fb06b3          	add	a3,s6,a5
    800061ce:	00068c23          	sb	zero,24(a3)
    idx[i] = alloc_desc();
    800061d2:	c21c                	sw	a5,0(a2)
    if(idx[i] < 0){
    800061d4:	0207c563          	bltz	a5,800061fe <virtio_disk_rw+0x8c>
  for(int i = 0; i < 3; i++){
    800061d8:	2485                	addiw	s1,s1,1
    800061da:	0711                	addi	a4,a4,4
    800061dc:	1f548a63          	beq	s1,s5,800063d0 <virtio_disk_rw+0x25e>
    idx[i] = alloc_desc();
    800061e0:	863a                	mv	a2,a4
  for(int i = 0; i < NUM; i++){
    800061e2:	0001c697          	auipc	a3,0x1c
    800061e6:	b3668693          	addi	a3,a3,-1226 # 80021d18 <disk>
    800061ea:	87d2                	mv	a5,s4
    if(disk.free[i]){
    800061ec:	0186c583          	lbu	a1,24(a3)
    800061f0:	fde9                	bnez	a1,800061ca <virtio_disk_rw+0x58>
  for(int i = 0; i < NUM; i++){
    800061f2:	2785                	addiw	a5,a5,1
    800061f4:	0685                	addi	a3,a3,1
    800061f6:	ff779be3          	bne	a5,s7,800061ec <virtio_disk_rw+0x7a>
    idx[i] = alloc_desc();
    800061fa:	57fd                	li	a5,-1
    800061fc:	c21c                	sw	a5,0(a2)
      for(int j = 0; j < i; j++)
    800061fe:	02905a63          	blez	s1,80006232 <virtio_disk_rw+0xc0>
        free_desc(idx[j]);
    80006202:	f9042503          	lw	a0,-112(s0)
    80006206:	00000097          	auipc	ra,0x0
    8000620a:	cfa080e7          	jalr	-774(ra) # 80005f00 <free_desc>
      for(int j = 0; j < i; j++)
    8000620e:	4785                	li	a5,1
    80006210:	0297d163          	bge	a5,s1,80006232 <virtio_disk_rw+0xc0>
        free_desc(idx[j]);
    80006214:	f9442503          	lw	a0,-108(s0)
    80006218:	00000097          	auipc	ra,0x0
    8000621c:	ce8080e7          	jalr	-792(ra) # 80005f00 <free_desc>
      for(int j = 0; j < i; j++)
    80006220:	4789                	li	a5,2
    80006222:	0097d863          	bge	a5,s1,80006232 <virtio_disk_rw+0xc0>
        free_desc(idx[j]);
    80006226:	f9842503          	lw	a0,-104(s0)
    8000622a:	00000097          	auipc	ra,0x0
    8000622e:	cd6080e7          	jalr	-810(ra) # 80005f00 <free_desc>
    sleep(&disk.free[0], &disk.vdisk_lock);
    80006232:	85e2                	mv	a1,s8
    80006234:	0001c517          	auipc	a0,0x1c
    80006238:	afc50513          	addi	a0,a0,-1284 # 80021d30 <disk+0x18>
    8000623c:	ffffc097          	auipc	ra,0xffffc
    80006240:	e2e080e7          	jalr	-466(ra) # 8000206a <sleep>
  for(int i = 0; i < 3; i++){
    80006244:	f9040713          	addi	a4,s0,-112
    80006248:	84ce                	mv	s1,s3
    8000624a:	bf59                	j	800061e0 <virtio_disk_rw+0x6e>
  ...
  if(write)
    ...
    8000624c:	00a60793          	addi	a5,a2,10 # 100a <_entry-0x7fffeff6>
    80006250:	00479693          	slli	a3,a5,0x4
    80006254:	0001c797          	auipc	a5,0x1c
    80006258:	ac478793          	addi	a5,a5,-1340 # 80021d18 <disk>
    8000625c:	97b6                	add	a5,a5,a3
    8000625e:	4685                	li	a3,1
    80006260:	c794                	sw	a3,8(a5)
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    80006262:	0001c597          	auipc	a1,0x1c
    80006266:	ab658593          	addi	a1,a1,-1354 # 80021d18 <disk>
    8000626a:	00a60793          	addi	a5,a2,10
    8000626e:	0792                	slli	a5,a5,0x4
    80006270:	97ae                	add	a5,a5,a1
    80006272:	0007a623          	sw	zero,12(a5)
  buf0->sector = sector;
    80006276:	0197b823          	sd	s9,16(a5)

  disk.desc[idx[0]].addr = (uint64) buf0;
    8000627a:	f6070693          	addi	a3,a4,-160
    8000627e:	619c                	ld	a5,0(a1)
    80006280:	97b6                	add	a5,a5,a3
    80006282:	e388                	sd	a0,0(a5)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    80006284:	6188                	ld	a0,0(a1)
    80006286:	96aa                	add	a3,a3,a0
    80006288:	47c1                	li	a5,16
    8000628a:	c69c                	sw	a5,8(a3)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    8000628c:	4785                	li	a5,1
    8000628e:	00f69623          	sh	a5,12(a3)
  disk.desc[idx[0]].next = idx[1];
    80006292:	f9442783          	lw	a5,-108(s0)
    80006296:	00f69723          	sh	a5,14(a3)

  disk.desc[idx[1]].addr = (uint64) b->data;
    8000629a:	0792                	slli	a5,a5,0x4
    8000629c:	953e                	add	a0,a0,a5
    8000629e:	05890693          	addi	a3,s2,88
    800062a2:	e114                	sd	a3,0(a0)
  disk.desc[idx[1]].len = BSIZE;
    800062a4:	6188                	ld	a0,0(a1)
    800062a6:	97aa                	add	a5,a5,a0
    800062a8:	40000693          	li	a3,1024
    800062ac:	c794                	sw	a3,8(a5)
  if(write)
    800062ae:	100d0d63          	beqz	s10,800063c8 <virtio_disk_rw+0x256>
    disk.desc[idx[1]].flags = 0; // device reads b->data
    800062b2:	00079623          	sh	zero,12(a5)
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    800062b6:	00c7d683          	lhu	a3,12(a5)
    800062ba:	0016e693          	ori	a3,a3,1
    800062be:	00d79623          	sh	a3,12(a5)
  disk.desc[idx[1]].next = idx[2];
    800062c2:	f9842583          	lw	a1,-104(s0)
    800062c6:	00b79723          	sh	a1,14(a5)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    800062ca:	0001c697          	auipc	a3,0x1c
    800062ce:	a4e68693          	addi	a3,a3,-1458 # 80021d18 <disk>
    800062d2:	00260793          	addi	a5,a2,2
    800062d6:	0792                	slli	a5,a5,0x4
    800062d8:	97b6                	add	a5,a5,a3
    800062da:	587d                	li	a6,-1
    800062dc:	01078823          	sb	a6,16(a5)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    800062e0:	0592                	slli	a1,a1,0x4
    800062e2:	952e                	add	a0,a0,a1
    800062e4:	f9070713          	addi	a4,a4,-112
    800062e8:	9736                	add	a4,a4,a3
    800062ea:	e118                	sd	a4,0(a0)
  ...
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    ...
    800062f6:	00a71623          	sh	a0,12(a4)
  disk.desc[idx[2]].next = 0;
    800062fa:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    800062fe:	00b92223          	sw	a1,4(s2)
  disk.info[idx[0]].b = b;
    80006302:	0127b423          	sd	s2,8(a5)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    80006306:	6698                	ld	a4,8(a3)
    80006308:	00275783          	lhu	a5,2(a4)
    8000630c:	8b9d                	andi	a5,a5,7
    8000630e:	0786                	slli	a5,a5,0x1
    80006310:	97ba                	add	a5,a5,a4
    80006312:	00c79223          	sh	a2,4(a5)
  ...
  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
  ...
  while(b->disk == 1) {
    80006332:	00492703          	lw	a4,4(s2)
    80006336:	4785                	li	a5,1
    80006338:	02f71163          	bne	a4,a5,8000635a <virtio_disk_rw+0x1e8>
    sleep(b, &disk.vdisk_lock);
    8000633c:	0001c997          	auipc	s3,0x1c
    80006340:	b0498993          	addi	s3,s3,-1276 # 80021e40 <disk+0x128>
  while(b->disk == 1) {
    80006344:	4485                	li	s1,1
    sleep(b, &disk.vdisk_lock);
    80006346:	85ce                	mv	a1,s3
    80006348:	854a                	mv	a0,s2
    8000634a:	ffffc097          	auipc	ra,0xffffc
    8000634e:	d20080e7          	jalr	-736(ra) # 8000206a <sleep>
  while(b->disk == 1) {
    80006352:	00492783          	lw	a5,4(s2)
    80006356:	fe9788e3          	beq	a5,s1,80006346 <virtio_disk_rw+0x1d4>
  }

  disk.info[idx[0]].b = 0;
    8000635a:	f9042903          	lw	s2,-112(s0)
    8000635e:	00290793          	addi	a5,s2,2
    80006362:	00479713          	slli	a4,a5,0x4
    80006366:	0001c797          	auipc	a5,0x1c
    8000636a:	9b278793          	addi	a5,a5,-1614 # 80021d18 <disk>
    8000636e:	97ba                	add	a5,a5,a4
    80006370:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    80006374:	0001c997          	auipc	s3,0x1c
    80006378:	9a498993          	addi	s3,s3,-1628 # 80021d18 <disk>
    8000637c:	00491713          	slli	a4,s2,0x4
    80006380:	0009b783          	ld	a5,0(s3)
    80006384:	97ba                	add	a5,a5,a4
    80006386:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    8000638a:	854a                	mv	a0,s2
    8000638c:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    80006390:	00000097          	auipc	ra,0x0
    80006394:	b70080e7          	jalr	-1168(ra) # 80005f00 <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    80006398:	8885                	andi	s1,s1,1
    8000639a:	f0ed                	bnez	s1,8000637c <virtio_disk_rw+0x20a>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    8000639c:	0001c517          	auipc	a0,0x1c
    800063a0:	aa450513          	addi	a0,a0,-1372 # 80021e40 <disk+0x128>
    800063a4:	ffffb097          	auipc	ra,0xffffb
    800063a8:	8fa080e7          	jalr	-1798(ra) # 80000c9e <release>
}
    ...
    800063c0:	6ce2                	ld	s9,24(sp)
    800063c2:	6d42                	ld	s10,16(sp)
    800063c4:	6165                	addi	sp,sp,112
    800063c6:	8082                	ret
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
    800063c8:	4689                	li	a3,2
    800063ca:	00d79623          	sh	a3,12(a5)
    800063ce:	b5e5                	j	800062b6 <virtio_disk_rw+0x144>
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    800063d0:	f9042603          	lw	a2,-112(s0)
    800063d4:	00a60713          	addi	a4,a2,10
    800063d8:	0712                	slli	a4,a4,0x4
    800063da:	0001c517          	auipc	a0,0x1c
    800063de:	94650513          	addi	a0,a0,-1722 # 80021d20 <disk+0x8>
    800063e2:	953a                	add	a0,a0,a4
  if(write)
    800063e4:	e60d14e3          	bnez	s10,8000624c <virtio_disk_rw+0xda>
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
    800063e8:	00a60793          	addi	a5,a2,10
    800063ec:	00479693          	slli	a3,a5,0x4
    800063f0:	0001c797          	auipc	a5,0x1c
    800063f4:	92878793          	addi	a5,a5,-1752 # 80021d18 <disk>
    800063f8:	97b6                	add	a5,a5,a3
    800063fa:	0007a423          	sw	zero,8(a5)
    800063fe:	b595                	j	80006262 <virtio_disk_rw+0xf0>

0000000080006400 <virtio_disk_intr>:
...
virtio_disk_intr()
{
  ...
  acquire(&disk.vdisk_lock);
    8000640a:	0001c497          	auipc	s1,0x1c
    8000640e:	90e48493          	addi	s1,s1,-1778 # 80021d18 <disk>
    80006412:	0001c517          	auipc	a0,0x1c
    80006416:	a2e50513          	addi	a0,a0,-1490 # 80021e40 <disk+0x128>
    8000641a:	ffffa097          	auipc	ra,0xffffa
    8000641e:	7d0080e7          	jalr	2000(ra) # 80000bea <acquire>
  ...
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80006422:	10001737          	lui	a4,0x10001
    80006426:	533c                	lw	a5,96(a4)
    80006428:	8b8d                	andi	a5,a5,3
    8000642a:	d37c                	sw	a5,100(a4)
  ...
  while(disk.used_idx != disk.used->idx){
    80006430:	689c                	ld	a5,16(s1)
    80006432:	0204d703          	lhu	a4,32(s1)
    80006436:	0027d783          	lhu	a5,2(a5)
    8000643a:	04f70863          	beq	a4,a5,8000648a <virtio_disk_intr+0x8a>
    __sync_synchronize();
    ...
    80006450:	00278713          	addi	a4,a5,2
    80006454:	0712                	slli	a4,a4,0x4
    80006456:	9726                	add	a4,a4,s1
    80006458:	01074703          	lbu	a4,16(a4) # 10001010 <_entry-0x6fffeff0>
    8000645c:	e721                	bnez	a4,800064a4 <virtio_disk_intr+0xa4>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    8000645e:	0789                	addi	a5,a5,2
    80006460:	0792                	slli	a5,a5,0x4
    80006462:	97a6                	add	a5,a5,s1
    80006464:	6788                	ld	a0,8(a5)
    b->disk = 0;   // disk is done with buf
    80006466:	00052223          	sw	zero,4(a0)
    wakeup(b);
    8000646a:	ffffc097          	auipc	ra,0xffffc
    8000646e:	c64080e7          	jalr	-924(ra) # 800020ce <wakeup>
  ...
  while(disk.used_idx != disk.used->idx){
    ...
    80006486:	faf71ce3          	bne	a4,a5,8000643e <virtio_disk_intr+0x3e>
  }

  release(&disk.vdisk_lock);
    8000648a:	0001c517          	auipc	a0,0x1c
    8000648e:	9b650513          	addi	a0,a0,-1610 # 80021e40 <disk+0x128>
    80006492:	ffffb097          	auipc	ra,0xffffb
    80006496:	80c080e7          	jalr	-2036(ra) # 80000c9e <release>
}
    ...
    800064a8:	3ac50513          	addi	a0,a0,940 # 80008850 <syscalls+0x400>
    800064ac:	ffffa097          	auipc	ra,0xffffa
    800064b0:	098080e7          	jalr	152(ra) # 80000544 <panic>
	...
