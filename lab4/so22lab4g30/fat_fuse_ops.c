...
static void fat_fuse_log_activity(char *operation_type, fat_file file) {
    ...
    fat_file log = fat_tree_get_file(node);
    off_t offset = log->dentry->file_size;
    fat_file parent_t = fat_tree_get_parent(node);

    if (fat_file_pwrite(log,buf,message_size,offset,parent_t) < 0)
       fat_error("Error writing %c file\n",BB_LOG_FILE);
}
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    u32 orphan_cluster_nummer = search_bb_orphan_dir_cluster();
    u32 aux_cluster = orphan_cluster_nummer;

    if(orphan_cluster_nummer == 0)
        orphan_cluster_nummer = fat_table_get_next_free_cluster(vol->table);    
    
    bb_init_log_dir(orphan_cluster_nummer); 
...
int fat_fuse_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    fat_fuse_log_activity(BB_LOG_FILE,file);
    return bytes_read;
}
...
int fat_fuse_unlink(const char *path){
    ...
    //Marcar como libres cluster
    set_cluster_free(vol->table,file,parent);
    fat_fuse_truncate(file->filepath,file->dentry->file_size);
