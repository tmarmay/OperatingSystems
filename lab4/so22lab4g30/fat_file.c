...
void set_cluster_free(fat_table table,fat_file file, fat_file parent){
    ...
    u32 aux_cluster;
    while(!fat_table_cluster_is_EOC(next_cluster)){
        aux_cluster = fat_table_get_next_cluster(table, next_cluster);
        next_cluster = FAT_CLUSTER_FREE;
        next_cluster = aux_cluster;
    }
